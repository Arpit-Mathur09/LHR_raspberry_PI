...
from hardware import PWMDevice, FanController, PIDController, SensorManager, LightController, PipetteManager
...
class RobotClient:
    ...
    def connect_wifi(self, ssid, password): 
        self.command_queue.put(("CONNECT_WIFI", (ssid, password)))
    ...
    def get_telemetry_snapshot(self):
        ...
        payload = {
            ...
            "started_by": self.state["started_by"], "calib_active": self.state["calibration_active"],
            "calibration_source": self.state["calibration_source"], "calib_status": self.state["calib_status"],
            "is_calibrated": self.state["is_calibrated"], "light_on": self.state["light_on"],
            "sensors": self.state["sensor_data"], "lid_open": self.state["lid_open"], "pipettes": self.state["pipettes"]
        }
        ...
        while True:
            ...
            if time.time() - last_pipette_check > 2.0:
                if self.pipettes.scan_pipettes():
                    # --- CRITICAL FIX: MUTATE IN-PLACE TO PRESERVE UI POINTER REFERENCES ---
            ...
            try:
                while not self.command_queue.empty():
                    ...
                    if cmd_type == "MANUAL" and self.ser: self.ser.write((data + "\n").encode())
                    ...
                    elif cmd_type == "DOWNLOAD_AND_RUN": fname, source = data; self.load_local_protocol(fname, source); waiting_for_response = False
                    elif cmd_type == "TOGGLE_LIGHT": self.toggle_light()
                        ...
                        should_pause = True if cmd_type == "REMOTE_PAUSE" else (False if cmd_type == "REMOTE_RESUME" else not self.is_paused)
